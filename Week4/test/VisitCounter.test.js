...
describe("VisitCounter", function() {
    ...
    describe("Visits", function() {
        describe("Record visits", function() {
            ...
            expect(await visitCounter.getCount()).to.deep.equal(ethers.BigNumber.from("1"));
            });
            ...
            it("Multiple users record multiple visits and counter increases", async function() {
                ...
                expect(await visitCounter.getCount()).to.deep.equal(ethers.BigNumber.from("10"));
                });
        });
        ...
        describe("Events", function() {
            it("User's visit emits an event", async function () {
                ...
                timeOffset = 1
                await expect(visitCounter.connect(visitor1).visit()).to.emit(visitCounter, "NewVisitor").withArgs(visitor1.address, await time.latest() + timeOffset);
